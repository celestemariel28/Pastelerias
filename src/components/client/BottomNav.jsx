...
export default function BottomNav({ 
  ...
  return (
    <nav 
      className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md lg:max-w-xl z-50 bg-white/95 backdrop-blur-md border-t border-primary-clear-b"
            ...
            >
              <div className="relative">
                ...
                {Boolean(item.badge && item.badge > 0) && (
                  <span className="absolute -top-1.5 -right-2.5 bg-primary text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center shadow-sm animate-scaleIn">
                    {item.badge > 99 ? '+99' : item.badge}
                  </span>
                )}
              </div>

              <span className={`text-[10px] tracking-tight mt-0.5 ${isActive ? '!text-primary font-black' : 'text-gray-500 font-medium'}`}>
                {item.label}
              </span>

              {isActive && (
                <span className="w-1.5 h-1.5 bg-primary rounded-full mt-0.5 animate-fadeIn" />
              )}
            </button>
          );
        })}
